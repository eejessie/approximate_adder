...
using namespace std;
int compute_G(int size, unsigned long long add1, unsigned long long add2)
{
	int G = add1 & add2;
	if (G == 0)
		return 0;
...
int compute_block_pgk(int m, int k, int l, int index, unsigned long long add1_low, unsigned long long add1_high, unsigned long long add2_low, unsigned long long add2_high)
{
//	cout << endl << "in compute_block_pgk, index = " << index << endl;
	...
	{
		int res = compute_G(size, add1_tmp, add2_tmp);
		if (res)
			return 2;                                  //group_G = 1
		else
			return 3;                                  //group_K = 1
...
void bb_adder(int n, int k, int l, unsigned long long add1_low, unsigned long long add1_high, unsigned long long add2_low, unsigned long long add2_high, unsigned long long &sum_est_low, unsigned long long &sum_est_high)
{
	...
	{	
		...
		//compute co
		int res = compute_block_pgk(m, k, l, i+1, add1_low, add1_high, add2_low, add2_high);
		if (res == 1)      co = 0;
		else if (res == 2) co = 1;
		else               co = 0;
